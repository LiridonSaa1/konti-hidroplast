...
function QualityControlGalleryPage() {
  ...
  const currentCategory = categories.find(
    (cat) =>
      cat.title.toLowerCase().includes("quality") ||
      cat.title.toLowerCase().includes("control"),
  );
  ...
  useEffect(() => {
    document.title = `${t("gallery.qualityControl")} ${t("gallery.title")} - Konti Hidroplast`;
  }, [t]);
          ...
          <div className="text-center">
            ...
            <div className="mb-6 bg-[#ef4444] text-white px-4 py-2 rounded-full inline-block">
              <span className="text-sm font-medium">
                {t("gallery.precisionExcellence")}
              </span>
            </div>
            <h1 className="text-5xl md:text-6xl font-bold mb-8 leading-tight">
              <span className="text-red-500">{t("gallery.qualityControl")}</span>
              <br />
              <span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">
                {t("gallery.qualityControlTitle")}
              </span>
            </h1>
            <p className="text-xl text-gray-300 mb-8 leading-relaxed max-w-3xl mx-auto">
              {t("gallery.qualityControlDescription")}
            </p>
            <div className="flex justify-center flex-wrap gap-4">
              <div className="flex items-center gap-2 text-blue-300">
                <Check className="w-5 w-5" />
                <span className="text-sm font-medium">{t("gallery.iso9001")}</span>
              </div>
              <div className="flex items-center gap-2 text-green-300">
                <Check className="w-5 w-5" />
                <span className="text-sm font-medium">{t("gallery.laboratoryTesting")}</span>
              </div>
              <div className="flex items-center gap-2 text-yellow-300">
                <Check className="w-5 w-5" />
                <span className="text-sm font-medium">{t("gallery.monitoring24_7")}</span>
              </div>
            </div>
          </div>
        </div>
